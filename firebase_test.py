...
try: 
    ...
    scoreseries = st.session_state.adj_df[score_list].groupby(by='unit').sum(axis=1)/st.session_state.adj_df[score_list].groupby(by='unit').count()[temp_list[2]]
...
with tab4:
    st.plotly_chart(px.bar(df_remain,title='Remaining Patrols at each station',text_auto=True))
